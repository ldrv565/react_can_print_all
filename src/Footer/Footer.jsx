import React from 'react'
import Modal from '../Modal/Modal.jsx';
import Modal__order from '../Modal/Modal__order/Modal__order.jsx';
import Modal__orderDesign from '../Modal/Modal__orderDesign/Modal__orderDesign.jsx';
import Modal__login from '../Modal/Modal__login/Modal__login.jsx';
const Footer = props => {

  return (
    <footer>
        <Modal>
            <Modal__order/>
            <Modal__orderDesign/>
            <Modal__login/>
        </Modal>
        <div className="footer">
            <a className="logo footer__logo" href="index.html">
                <img src="img/logo.png" alt="logo"/>
            </a>
            <div className="footer__item">
                <ul>
                    <li><strong>Let us help</strong></li>
                    <a href="index.html">
                        <li>My Account</li>
                    </a>
                    <a href="index.html">
                        <li>Help</li>
                    </a>
                    <a href="index.html">
                        <li>Contact us</li>
                    </a>
                    <a href="index.html">
                        <li>Shipping & delivery</li>
                    </a>
                    <a href="index.html">
                        <li>Request samples</li>
                    </a>
                </ul>
            </div>
            <div className="footer__item">
                <ul>
                    <li><strong>What we offer</strong></li>
                    <a href="index.html">
                        <li>Our products</li>
                    </a>
                    <a href="index.html">
                        <li>Upload your designs</li>
                    </a>
                    <a href="index.html">
                        <li>Partner with us</li>
                    </a>
                    <a href="index.html">
                        <li>Advertise with us</li>
                    </a>
                    <a href="index.html">
                        <li>Reseller program</li>
                    </a>
                    <a href="index.html">
                        <li>Popular searches</li>
                    </a>
                </ul>
            </div>
            <div className="footer__item">
                <ul>
                    <li><strong>Our policies</strong></li>
                    <a href="index.html">
                        <li>Copyright matters</li>
                    </a>
                    <a href="index.html">
                        <li>Trademark matters</li>
                    </a>
                    <a href="index.html">
                        <li>Patents & trademarks</li>
                    </a>
                </ul>
            </div>
            <div className="footer__item">
                <ul>
                    <li><strong>Our company</strong></li>
                    <a href="index.html">
                        <li>About us</li>
                    </a>
                    <a href="index.html">
                        <li>Careers</li>
                    </a>
                    <a href="index.html">
                        <li>For media</li>
                    </a>
                    <a href="index.html">
                        <li>For investors</li>
                    </a>
                    <a href="index.html">
                        <li>Sustainability</li>
                    </a>
                    <a href="index.html">
                        <li>Webs</li>
                    </a>
                </ul>
            </div>
            <div className="footer__footer">
                | <a href="index.html"><strong>Home</strong></a>|<a href="index.html"><strong>Site Map</strong></a>
                | <a href="index.html"><strong>Privacy Policy</strong></a> | <a href="index.html"><strong>Terms of Use</strong></a>
                | <a href="index.html"><strong>a CIMPRESS company</strong></a>
            </div>
            <div className="footer__footer">
                <p>© 2018 All rights reserved.</p>
            </div>
        </div>
    </footer>
  )
}


export default Footer